...
local g_ScrollOffset = 0

local g_LingYuData = {}
-- local g_Lingyu_Equip_MouseEnter_Data = {}
...
function Lingyu_OnScrollPosChanged(idx)
	if idx == 1 then
		-- 滚动条位置改变
		local nIndex = math.floor(Lingyu_ListContent_Scrollbar:GetPosition())
		g_ScrollOffset = nIndex
		
		-- 更新显示内容（不移动容器）
		Lingyu_UpdateVisibleItems()
		
		Lingyu_Sync_Scrollbar_Position(2, nIndex)
	elseif idx == 2 then
		-- 鼠标滚轮事件
		local attr_list_count = LuaFnGetLingYuAttrCount()
		if attr_list_count > 10 then
			-- 计算滚动方向和新位置
			local delta = 1  -- 每次滚动1行
			if event and event == "MouseWheel" then
				delta = -arg1  -- 鼠标滚轮方向
			end
			
			local max_scroll = attr_list_count - 10
			local new_offset = math.max(0, math.min(g_ScrollOffset + delta, max_scroll))
			
			if new_offset ~= g_ScrollOffset then
				g_ScrollOffset = new_offset
				Lingyu_ListContent_Scrollbar:SetPosition(g_ScrollOffset)
				Lingyu_UpdateVisibleItems()
			end
		end
	end
...
function Lingyu_UpdateVisibleItems()
	local attr_list_count = LuaFnGetLingYuAttrCount()
	...
	-- 计算当前可见范围（数据索引）
	local visible_start = g_ScrollOffset + 1
	local visible_end = math.min(visible_start + 9, attr_list_count)  -- 最多显示10个
	
	-- 显示可见范围内的属性（使用前10个UI元素）
	local ui_index = 1
	for data_index = visible_start, visible_end do
		if data_index <= attr_list_count and ui_index <= 10 then
			local strAttrName = LuaFnGetLingYuAttrName(data_index)
			local strAttrValue = LuaFnGetLingYuAttrValueString(data_index)
			
			g_LingYuAttrElement.item[ui_index]:Show()
			g_LingYuAttrElement.title[ui_index]:SetText(strAttrName)
			g_LingYuAttrElement.value[ui_index]:SetText(strAttrValue)
...
function Lingyu_Update()
	...
	for i = 1, 6 do
		...
		if LingYuID > 0 then
			...
			-- g_LingYuAction[i]:SetActionItem(LingYuID)
		end
	end
	...
	if attr_list_count > 0 then
		...
		if attr_list_count <= 10 then
			...
			-- 文档大小应该是超出可见区域的条目数
			Lingyu_ListContent_Scrollbar:SetProperty("DocumentSize", attr_list_count - 10)
		end
...
function Lingyu_Equip_MouseEnter(index)
	...
	local ActionLY
	for i = 1, 6 do
		LingYuID = LuaFnGetLingYuID(i)
		-- PushDebugMessage("LingYuID:"..tostring(LingYuID))
		ActionLY = GemMelting:UpdateProductAction(LingYuID)
		g_LingYuAction[i]:SetActionItem(ActionLY:GetID())
	end
	if LuaFnGetLingYuID(index) > 0 then	
		LingYuID = LuaFnGetLingYuID(index)
		ActionLY = GemMelting:UpdateProductAction(LingYuID)
		g_LingYuAction[index]:SetActionItem(ActionLY:GetID())
		local nLingYuData = LuaFnGetLingYuAttrData(index)
		if nLingYuData ~= "" then
			PushEvent("UI_COMMAND", 11605923, nLingYuData,LingYuID)
		end
	end
	
end
-- function Lingyu_Equip_MouseLeave(index)
	-- PushDebugMessage(112233)
...
-- function Lingyu_Equip_MouseEnter(index)
    ...
    -- if LuaFnGetLingYuID(index) > 0 then    
        ...
        -- local nLingYuData = LuaFnGetLingYuAttrData(index)
        -- if nLingYuData ~= "" then
            -- g_Lingyu_Equip_MouseEnter_Data[index] = {data=nLingYuData, id=LingYuID}
            -- SetTimer("Lingyu_Equip_DelayShow_"..index, "Lingyu_Equip_DelayShow("..index..")", 200)
        -- end
    -- end
-- end

-- function Lingyu_Equip_DelayShow(index)
    -- KillTimer("Lingyu_Equip_DelayShow_"..index.."()")
    -- local params = g_Lingyu_Equip_MouseEnter_Data[index]
    -- if params then
        -- PushEvent("UI_COMMAND", 11605923, params.data, params.id)
        -- g_Lingyu_Equip_MouseEnter_Data[index] = nil
    -- end
-- end
