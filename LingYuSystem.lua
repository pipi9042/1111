...
--打造产出规则
--[特殊材料ID] = {scale = {黄阶的数量, 玄阶的数量, 地阶的数量, 天阶的数量}}, attr_num = {扩展规则条数，将随机循环出一个}},
...
--装备类型基础属性信息
--第一维是品阶(1-4分别为黄、玄、地、天)
--第二维是装备类型(1-6分别为镜、刺、甲、钩、带、匕)
--第三维是属性值, 分别为血上限、内功、外攻、内防御、外防御、命中、闪避、会心
...
--装备类型扩展属性基准值信息
--属性编号1-30，对应上述30种属性，以属性编号为索引排序
--{增加HP的上限, 增加MP的上限, 冰攻击, 冰抵抗, 减冰抗, 火攻击, 火抵抗, 减火炕, 玄攻击, 玄抵抗, 减玄抗, 毒攻击, 毒抵抗, 减毒抗, 物理攻击, 物理防御, 魔法攻击, 魔法防御, 命中, 闪避, 会心一击（双倍攻击）的百分比, 力量, 灵气, 体质, 定力, 身法, 会心防御, 增加所有的人物一级属性, 伤害增加, 伤害降低}
...
--装备品阶记录
--[装备ID] = {product_type = 装备类型(1-6分别为镜、刺、甲、钩、带、匕), rank = 品阶(1-4分别为黄、玄、地、天)},
...
--[配方ID] = {stuff_id = 必备材料的ID, product_type = 装备类型(1-6分别为镜、刺、甲、钩、带、匕), product = {黄阶的ID, 玄阶的ID, 地阶的ID, 天阶的ID}}, 
...
function x888808_AbilityProduce(sceneId, selfId, recipeId)
    ...
    local nBagPos = LuaFnTryRecieveItem(sceneId, selfId, nItemID, QUALITY_MUST_BE_CHANGE) -- 放不下就没有了
    if nBagPos < 0 then
...
----------------------------------------------------------------------------------------
--	补全字符串，根据传入的参数将字符串补全到指定长度，长度不足的字符串将在前面补0
----------------------------------------------------------------------------------------
...
function x888808_TransferEnhanced(sceneId, selfId, srcPos, destPos)
    ...
    if nAttrCount2 ~= nAttrCount1 then
        x888808_NotifyTip(sceneId, selfId, "目标灵武与源灵武的扩展属性条数不一致，无法转移！" )
        return
    end
	...
	x888808_NotifyTip(sceneId, selfId, "铸炼属性转移成功。" )
...
function x888808_SaveEnhanced(sceneId, selfId, nLingYuPos)
	...
	x888808_NotifyTip(sceneId, selfId, "已成功替换铸炼属性。" )
	BeginUICommand(sceneId)
...
function x888808_ExtendedEnhance(sceneId, selfId, nLingYuPos, nItemPos)
	...
	if nItemID ~= x888808_g_EnhanceNeedItemList[nRank] then
		...
		elseif nRank == 4 and nItemID == x888808_g_EnhanceNeedItemList[6] then
			...
			x888808_NotifyTip(sceneId, selfId, format("#H铸炼天鉴灵武%s，需要%s。", GetItemName(sceneId, nLingYuID), GetItemName(sceneId, nItemID)))
			return
		end
	end
	local _, nLingYuAttrStr = LuaFnGetItemCreator(sceneId, selfId, nLingYuPos)
...
function x888808_Use_LingYu(sceneId, selfId, nLingYuPos)
    if nLingYuPos == nil or nLingYuPos < 0 then
		x888808_NotifyTip( sceneId, selfId, "无效的天鉴灵武装备，无法穿戴！" )
		return
	end
	...
	if not nNewLingYuID or nNewLingYuID <= 0 or not x888808_g_ProductRankInfo[nNewLingYuID] then
		--x888808_NotifyTip( sceneId, selfId, "天鉴灵武装备异常，无法穿戴！" )
		return
	end
	if nNewLingYuID and nNewLingYuID > 0 and x888808_g_ProductRankInfo[nNewLingYuID] then
	    ...
	    local nLingYuData = LuaFnReadLingYuData(sceneId, selfId)
		AddGlobalCountNews(sceneId, "nLingYuData[nProductType]:"..tostring(nLingYuData[nProductType]))
		AddGlobalCountNews(sceneId, "nLingYuData[nProductType].id:"..tostring(nLingYuData[nProductType].id))
	    if nLingYuData[nProductType] and nLingYuData[nProductType].id > 0 then
			...
			AddGlobalCountNews(sceneId, nNewPos)
	        LuaFnItemBind(sceneId,selfId,nNewPos);
...
function x888808_Disassembly(sceneId, selfId, nLingYuPos)
    if nLingYuPos == nil or nLingYuPos < 0 then
		x888808_NotifyTip( sceneId, selfId, "无效的天鉴灵武装备，无法兑换！" )
		return
	end
	local nLingYuID = LuaFnGetItemTableIndexByIndex(sceneId, selfId, nLingYuPos)
	...
	if not nProductID then
		x888808_NotifyTip( sceneId, selfId, "要兑换的漱玉泉异常，无法兑换！" )
		return
	end
	...
	x888808_NotifyTip(sceneId, selfId, format("#H成功兑换一个%s。", GetItemName(sceneId, nProductID)))
end
...
function x888808_Synthesis(sceneId, selfId, nItemPos, nNewItemID)
    if nItemPos == nil or nItemPos < 0 then
		x888808_NotifyTip( sceneId, selfId, "无效的漱玉泉，无法合成！" )
		return
	end
	local nItemID = LuaFnGetItemTableIndexByIndex(sceneId, selfId, nItemPos)
	...
	if LuaFnGetMaterialBagSpace( sceneId, selfId) < 1 then
		x888808_NotifyTip( sceneId, selfId,"材料栏至少需要1个空位")
		return
	end
	...
	if LuaFnGetAvailableItemCount(sceneId, selfId, nItemID) < nProductInfo.count then
		x888808_NotifyTip(sceneId, selfId, format("%s数量不足%s个。", GetItemName(sceneId, nItemID), nProductInfo.count)) 
		return
	end
	if LuaFnDelAvailableItem(sceneId,selfId, nItemID, nProductInfo.count ) ~= 1 then
		x888808_NotifyTip(sceneId, selfId, format("%s扣除失败。", GetItemName(sceneId, nItemID))) 
	    return
	end
	LuaFnCostMoneyWithPriority(sceneId, selfId, nProductInfo.money)
	LuaFnTryRecieveItem(sceneId, selfId, nNewItemID, 1)
	x888808_NotifyTip(sceneId, selfId, format("#H成功兑换一个%s。", GetItemName(sceneId, nNewItemID))) 
end
...
--x888808_NotifyTip( sceneId, selfId,"玩家登录初始化天鉴灵武")
...
function x888808_UpdateClientOtherLingYuData(sceneId, selfId, targetId)
	--AddGlobalCountNews(sceneId,"调试结果:#G"..LuaFnGuid2ObjId(sceneId,targetId)	)--打印世界
	local nPlayerTag = LuaFnGetGUID(sceneId, LuaFnGuid2ObjId(sceneId,targetId))
	--AddGlobalCountNews(sceneId,"调试结果:#G"..nPlayerTag	)--打印世界
    local nKitData = {}
    local nLingYuSkillId = 0
    local nLingYuKitRank = 5
    BeginUICommand(sceneId)
